...
function parseQuestionsFromText(text) {
  ...
  for (const b of blocks) {
    const qMatch = b.match(/^\s*\d+\.\s*(.+)/m);
    const a = b.match(/a\.\s*(.+)/i);
    const bb = b.match(/b\.\s*(.+)/i);
    const c = b.match(/c\.\s*(.+)/i);
    const d = b.match(/d\.\s*(.+)/i);
    const ans = b.match(/Answer:\s*([a-d])/i);

    if (qMatch && a && bb && c && d && ans) {
      items.push({
        q: qMatch[1].trim(),
        choices: [a[1].trim(), bb[1].trim(), c[1].trim(), d[1].trim()],
        answer: { a: 0, b: 1, c: 2, d: 3 }[ans[1].toLowerCase()],
      });
    }
  }
  return items;
...
// === Your 30 questions (unchanged) ===
const SAMPLE_TEXT = `1. What is the main mission of IncoNetwork in web3
